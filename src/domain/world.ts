...
export function makeWorld(init: WorldInit): World {
    return {
        ...
        enemyPattern: null,
        emitterPos: {
            x: init.bounds.x + init.bounds.w / 2,
            y: init.bounds.y + 8,
        },
        firingEnabled: true,
        nextId: 1,
...
/** 互換用の固定エミッタと、各アクティブ敵から弾を発射する。 */
function emitBullets(world: World, dt: number): void {
    if (world.firingEnabled) {
        // 固定エミッタ（レガシー用）＋ 各敵が自分の位置から発射
        if (world.enemyPattern)
            emitFrom(world, world.enemyPattern, world.emitterPos, dt);
        for (const e of world.enemies)
            if (e.pattern) emitFrom(world, e.pattern, e.pos, dt);
    }
    world.time += dt;
}

/** 弾を移動し、曲がる弾と画面端の跳ね返りを適用する。 */
function moveBullets(world: World, dt: number): void {
    for (const b of world.bullets) {
        if (
            b.angularVelocity &&
            (b.curveUntil === undefined || world.time < b.curveUntil)
        ) {
        ...
        if (bounced) {
            ...
            if (
                b.maxBounceSpeed &&
                Math.hypot(b.vel.x, b.vel.y) >= b.maxBounceSpeed
            ) {
        ...
        if (b.owner === "enemy") {
            if (
                ...
                circlesOverlap(
                    ...
                    b.radius,
                    world.ship.pos,
                    world.ship.hitRadius,
                )
            ) {
                events.push({
                    kind: "bullet-hits-ship",
                    bullet: b.id,
                    owner: "enemy",
                });
            ...
            if (
                !shipInvuln &&
                !b.grazed &&
                circlesOverlap(b.pos, b.radius, world.ship.pos, GRAZE_RADIUS)
            ) {
                b.grazed = true;
                events.push({
                    kind: "bullet-grazes-ship",
                    bullet: b.id,
                    owner: "enemy",
                });
            }
            ...
            const enemy = world.enemies.find(
                (e) => e.targetable !== false && bulletHitsEnemy(b, e),
            );
            if (enemy) {
                events.push({
                    kind: "bullet-hits-enemy",
                    bullet: b.id,
                    enemy: enemy.id,
                    owner: "player",
                });
...
function reflectPlayerBullet(
    ...
    const speed = Math.min(
        REFLECT_MAX_SPEED,
        Math.hypot(bullet.vel.x, bullet.vel.y) * 0.55,
    );
    const radius = Math.min(
        REFLECT_MAX_RADIUS,
        Math.max(2, bullet.radius * 0.45),
    );
    const returnAngle = Math.atan2(-bullet.vel.y, -bullet.vel.x);
    for (
        let lane = -Math.floor((n - 1) / 2);
        lane <= Math.floor((n - 1) / 2);
        lane++
    ) {
        const reflected =
            lane === 0
