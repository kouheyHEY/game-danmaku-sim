...
export const WEAPON_UPGRADES: WeaponUpgrade[] = [
    ...
    // 弾を大きく＝当たり幅UP。単発のままでも off-center の雑魚を捉えられる（拡散と同等の快適さ）
    {
        name: "弾を大きく",
        apply: (l) =>
            void (l.weapon.radius = Math.min(RADIUS_CAP, l.weapon.radius + 2)),
        available: (l) => l.weapon.radius < RADIUS_CAP,
    },
    {
        name: "連射UP",
        apply: (l) =>
            void (l.weapon.interval = Math.max(0.04, l.weapon.interval * 0.85)),
        available: (l) => l.weapon.interval > 0.05,
    },
    {
        name: "弾速UP",
        apply: (l) => void (l.weapon.speed += 100),
        available: (l) => l.weapon.speed < 1200,
    },
    ...
    // 弾数が増えたときも射線が画面外へ散りすぎないよう、角度強化は収束方向だけにする
    {
        name: "収束UP",
        apply: (l) =>
            void (l.weapon.spread = Math.max(
                SPREAD_FLOOR,
                l.weapon.spread - 0.03,
            )),
        available: (l) =>
            l.weapon.kind !== "straight" && l.weapon.spread > SPREAD_FLOOR,
    },
];
...
export function randomWeaponUpgrade(rng: Rng, loadout: PlayerLoadout): string {
    const pool = WEAPON_UPGRADES.filter(
        (u) => !u.available || u.available(loadout),
    );
...
export function drawSpecialUpgrades(
    rng: Rng,
    loadout: PlayerLoadout,
): SpecialUpgrade[] {
