...
たまに出るボスを倒すと **HP+1回復＆弾幕がランダム強化**。スコアは避けた弾数×倍率で、プリースト撃破ごとに倍率が1.1倍。
...
弾を避け続ける（右上に避けた弾数・倍率・スコア）→ たまにボスが出現 → 自弾で撃破すると
HP+1 と弾幕強化（弾数/連射/威力/弾速/収束をランダム）→ だんだん激しくなる。HP0 で終了。

**3体ごとのボスは強敵ボス。** 通常より硬く強い専用弾幕を持ち、撃破するとゲームが一時停止して
...
- **大ボス連戦**：`?debug&bossrush` で、雑魚・通常ボスなしの特徴ボス連戦モードになる。デバッグパネルの「大ボス連戦開始」からも開始可能。
