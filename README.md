...
たまに出る大ボスを倒すと **HP+1回復＆特別強化の2択**。スコアは避けた弾数×倍率で、プリースト撃破ごとに倍率が1.1倍。
...
弾を避け続ける（右上に避けた弾数・倍率・スコア）→ 雑魚がいなくなると大ボスが出現 → 自弾で撃破すると
...
- **大ボス連戦**：`?debug&bossrush` で、雑魚なしの大ボス連戦モードになる。デバッグパネルの「大ボス連戦開始」からも開始可能。
